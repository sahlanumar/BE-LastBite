...
public class OrderMapper {
    ...
    // ===> METODE BARU YANG PERLU DITAMBAHKAN <===
    /**
     * Mengubah objek CartItem menjadi entitas OrderItem.
     * Diperlukan untuk fungsionalitas checkout dari keranjang.
     * @param cartItem Item dari keranjang belanja.
     * @return Entitas OrderItem yang siap disimpan.
     */
    public static OrderItem toOrderItemEntity(CartItem cartItem) {
        // Mengambil quantity dari CartItem dan price dari MenuItem yang terkait
        return OrderItem.builder()
                .menuItem(cartItem.getMenuItem())
                .quantityPurchased(cartItem.getQuantity())
                .pricePerItem(cartItem.getMenuItem().getDiscountedPrice())
                .build();
    }
    ...
    public static Order toOrderEntity(User customer, SellerProfile seller, List<OrderItem> items, BigDecimal totalAmount, String verificationCode) {
        Order order = Order.builder()
                .customer(customer)
                .sellerProfile(seller) // Nama parameter di sini bisa 'seller' atau 'sellerProfile', yang penting tipenya SellerProfile
                .orderItems(items)
    ...
    public static OrderResponse toResponse(Order order, PaymentResponse payment) {
        ...
        // Di service, Anda memanggil toResponse(savedOrder, paymentResponse)
        // Mapper ini akan menempatkan paymentResponse ke field 'payment' di OrderResponse
